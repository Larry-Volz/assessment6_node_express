...
async function getOneDeveloper(developer){
  await axios.get(`https://api.github.com/users/${developer}`)
      // .then(developerData => console.log("DEVELOPER DATA", developerData))
      .then(developerData =>developerArr.push(developerData))
      .then(x => {return developerArr})

      //Respond to user with contents of that array [ {name, bio}, ... ]
      .catch(err => {throw new ExpressError(errorResp, 401)})
}

function getDevelopers(userReq) {
  let errorResp = `**************************************************************************\r\n
                   *                                                                         *\r\n
  ...
  for (developer of userReq){
    try{
      ...
      developerArr = getOneDeveloper(developer);
      console.log(typeof developerArr);
      debugger;
